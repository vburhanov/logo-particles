...
export default class Particles {
	...
	initPoints(discard) {
		...
		const uniforms = {
			...
			uTexture: { type: "t", value: new THREE.ImageUtils.loadTexture("images/texture.png") },
			uTouch: { value: null },
			// uMap: {value: new THREE.ImageUtils.loadTexture("image/texture.png")},
		};
...
// uvs
	...
	initHitArea() {
		...
		const material = new THREE.MeshBasicMaterial({ color: 0xdddccc, wireframe: true, depthTest: false });
	...
	show(time = 1.0) {
		// reset
		TweenLite.fromTo(this.object3D.material.uniforms.uSize, time, { value: 0.9 }, { value: 1.5 });
		TweenLite.to(this.object3D.material.uniforms.uRandom, time, { value: 2.0 });
		TweenLite.fromTo(this.object3D.material.uniforms.uDepth, time * 3.5, { value: 20.0 }, { value: 4.0 });
